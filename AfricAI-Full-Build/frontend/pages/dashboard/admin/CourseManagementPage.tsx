...
const CourseManagementPage: React.FC = () => {
    ...
    const filteredCourses = useMemo(() => {
        ...
        return courses.filter(course =>
            ...
            (course.tutor && course.tutor.name.toLowerCase().includes(searchTerm.toLowerCase()))
        );
    }, [courses, searchTerm]);
    ...
    const tableData = filteredCourses.map(course => [
        ...
        `⭐ ${course.rating?.toFixed(1) || 'N/A'} (${course.reviews} reviews)`,
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <h1 className="text-4xl font-bold">Course Management</h1>
                 <div className="relative">
                    <input type="text" placeholder="Search by title or tutor..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="bg-white/5 border border-white/10 rounded-md py-2 pl-10 pr-4 text-sm w-64 focus:ring-2 focus:ring-nebula-600 focus:outline-none" />
                    <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                </div>
            </div>
            
            <Table title={`All Courses (${filteredCourses.length})`} headers={tableHeaders} data={tableData} />

            {filteredCourses.length === 0 && (
                <div className="bg-black/20 backdrop-blur-lg border border-white/10 rounded-xl p-8 text-center text-gray-400">
